rankall <- function(outcome, num = "best") {
  ...
  frame0 <-set0[nom]
  state <- unique(frame0[2])
  state <- state[sort.list(state[,1],na.last = TRUE, decreasing = FALSE),]
  #outcome <- "pneumonia"
  #num <- "best"
  if (outcome == "heart failure"){
    frame1 <- frame0[frame0$State == state[1],]
    ...
    set0 <- frame2[frame2$Hospital.30.Day.Death..Mortality..Rates.from.Heart.Failure == rank2[num], ]
    ...
    if (num == "worst"){
      numm <- length(frame2[,2])
    }
    partial <- set0[numm,]
    for (s in 2:length(state)){
      frame1 <- frame0[frame0$State == state[s],]
      frame2 <- frame1[c(nom[1],nom[4],nom[2])]
      frame2[,2] <- as.numeric(frame2[,2])
      frame2 <- frame2[!rowSums(is.na(frame2[2])), ]
      rank <- unique(frame2[2])
      ...
      partial1 <- set0[numm,]
      partial2 <- rbind(partial,partial1)
      partial <- partial2
    }
    partial[,3] <- state
    return (partial)
